...
export default function ModalEditarProduto({ produto, onClose, onAtualizar }: ModalEditarProdutoProps) {
  const [mensagemSucesso, setMensagemSucesso] = useState('');
  

  const [nome, setNome] = useState(produto.nome);
  const [descricao, setDescricao] = useState(produto.descricao);
  const [quantidade, setQuantidade] = useState<number | ''>(produto.estoque);
  


  const precoFormatadoInicial = new Intl.NumberFormat('pt-BR', {
    style: 'currency',
    currency: 'BRL',
  }).format(produto.preco);
  const [preco, setPreco] = useState(precoFormatadoInicial);
    ...
    async function carregarSubCategorias() {
      try {
        ...
        if (response.ok) {
          ...
          // vai tentar pré-selecionar a subcategoria se o produto tiver uma
          if (produto.subcategoriaId) {
            const sub = dados.find((c: any) => c.id === produto.subcategoriaId);
            if (sub) setSubCategoriaSelecionada(sub);
          }
        } 
      } catch (error) {
  ...
  const handleSalvar = async () => {
    ...
    formData.append('lojaId', String(produto.loja.id)); 


    //  só anexa fotos se o usuário tiver feito upload de fotos novas
    ...
    try {
      ...
      await axios.put(`http://localhost:3001/produtos/${produto.id}`, formData, {
        headers: {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm font-spartan">
      ...
      <div className="relative w-[1020px] h-[1022px] max-h-[95vh] overflow-y-auto rounded-2xl bg-[#ededed] p-10 shadow-xl flex flex-col">
        ...
        <div className="w-full flex flex-col items-center pt-[15px]">
          ...
          <h2 className="w-[411px] h-[43px] text-[52px] text-black text-center leading-none">
            Editar Produto
          </h2>
          ...
          <label 
            ...
            <>
                ...
                <span className="text-[20px] font-medium text-[#7C3AED]">{foto1.name}</span>
                <span className="text-[14px] text-gray-500 mt-1">Clique para trocar a imagem nova</span></>) : 
                (<>
                <Image src="/icone_camera.png" alt="Ícone de upload" width={84} height={77} className="mb-3" />
                <span className="text-[20px] text-gray-800">Anexe as fotos do seu produto</span>
            </>
            )}
          ...
          <div className="flex w-[826px] gap-6 mt-[20px] justify-between">
            <label className={`flex flex-col items-center justify-center w-[228px] h-[147px] cursor-pointer transition-all duration-300 ${foto2 ? "border-2 border-solid border-[#7C3AED] bg-[#7C3AED]/5 shadow-sm rounded-[24px]" : "bg-[url('/Rectangle_small.png')] bg-center bg-[length:100%_100%] bg-no-repeat hover:opacity-80"}`}>
                {foto2 ? (<><div className="w-[45px] h-[45px] bg-[#7C3AED] rounded-full flex items-center justify-center mb-3 shadow-md"><span className="text-white text-xl font-bold">✓</span></div><span className="text-[20px] font-medium text-[#7C3AED] truncate px-2">{foto2.name}</span></>) : (<Image src="/icone_camera.png" alt="Ícone de upload" width={84} height={77} className="mb-3" />)}
                <input type="file" accept="image/*" className="hidden" onChange={(e) => setFoto2(e.target.files?.[0] || null)} />
            </label>
             <label className={`flex flex-col items-center justify-center w-[228px] h-[147px] cursor-pointer transition-all duration-300 ${foto3 ? "border-2 border-solid border-[#7C3AED] bg-[#7C3AED]/5 shadow-sm rounded-[24px]" : "bg-[url('/Rectangle_small.png')] bg-center bg-[length:100%_100%] bg-no-repeat hover:opacity-80"}`}>
                {foto3 ? (<><div className="w-[45px] h-[45px] bg-[#7C3AED] rounded-full flex items-center justify-center mb-3 shadow-md"><span className="text-white text-xl font-bold">✓</span></div><span className="text-[20px] font-medium text-[#7C3AED] truncate px-2">{foto3.name}</span></>) : (<Image src="/icone_camera.png" alt="Ícone de upload" width={84} height={77} className="mb-3" />)}
                <input type="file" accept="image/*" className="hidden" onChange={(e) => setFoto3(e.target.files?.[0] || null)} />
            </label>
             <label className={`flex flex-col items-center justify-center w-[228px] h-[147px] cursor-pointer transition-all duration-300 ${foto4 ? "border-2 border-solid border-[#7C3AED] bg-[#7C3AED]/5 shadow-sm rounded-[24px]" : "bg-[url('/Rectangle_small.png')] bg-center bg-[length:100%_100%] bg-no-repeat hover:opacity-80"}`}>
                {foto4 ? (<><div className="w-[45px] h-[45px] bg-[#7C3AED] rounded-full flex items-center justify-center mb-3 shadow-md"><span className="text-white text-xl font-bold">✓</span></div><span className="text-[20px] font-medium text-[#7C3AED] truncate px-2">{foto4.name}</span></>) : (<Image src="/icone_camera.png" alt="Ícone de upload" width={84} height={77} className="mb-3" />)}
                <input type="file" accept="image/*" className="hidden" onChange={(e) => setFoto4(e.target.files?.[0] || null)} />
            </label>
          </div>
          ...
          <div className={`relative mt-[15px] w-[826px] bg-white transition-all duration-300 overflow-hidden ${subCategoriaAberta ? 'rounded-[32px] pb-6 shadow-md z-20' : 'rounded-[99px] h-[65px] z-10'}`}>
            <div onClick={() => setSubCategoriaAberta(!subCategoriaAberta)} className="w-full h-[65px] px-8 flex items-center justify-between cursor-pointer">
            ...
            {subCategoriaAberta && (
                ...
                {subCategorias.map((cat) => (
                    <label key={cat.id} className="flex items-center gap-3 cursor-pointer group" onClick={() => handleSelecionarSubCategoria(cat)}>
                    <div className="relative w-[20px] h-[20px] rounded-full border-[2px] border-[#7C3AED] flex items-center justify-center">
                        {subCategoriaSelecionada?.id === cat.id && (<div className="w-[10px] h-[10px] bg-[#7C3AED] rounded-full"></div>)}
                    </div>
          ...
          <button 
            ...
            disabled={carregando}
            className="mt-[15px] w-[826px] h-[65px] rounded-full bg-red-600 text-white text-[24px] font-medium transition-colors cursor-pointer hover:bg-red-700 disabled:bg-gray-400"
          >
            DELETAR
          </button>

          <div className="flex items-center justify-center gap-[60px] mt-[15px] mb-[10px]">
            <button type="button" onClick={handleDiminuir} className="w-[78px] h-[78px] rounded-full border-[2px] border-[#7C3AED] flex items-center justify-center hover:bg-[#7C3AED]/10 transition-colors cursor-pointer group">
                <div className="w-[24px] h-[4px] bg-[#7C3AED] rounded-full group-hover:scale-110 transition-transform"></div>
            </button>

            <input type="text" value={quantidade} onChange={handleChangeQuantidade} onBlur={handleBlurQuantidade} className="w-[150px] text-center text-[65px] font-medium text-[#7C3AED] bg-transparent outline-none font-spartan" />


            <button type="button" onClick={handleAumentar} className="w-[78px] h-[78px] rounded-full border-[2px] border-[#7C3AED] flex items-center justify-center hover:bg-[#7C3AED]/10 transition-colors cursor-pointer relative group">
