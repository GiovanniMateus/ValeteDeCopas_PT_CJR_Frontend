  ...
  return (
    <div className="flex gap-1">
      {Array.from({ length: 5 }).map((_, i) => (
        <span key={i} className={`text-4xl ${i < nota ? "text-yellow-300" : "text-gray-500"}`}>
          ★
        </span>
      ))}
    </div>
...
export default function AvaliacaoLojaPage() {
  ...
  async function carregarDados() {
    try {
      const [avaliacaoRes, comentariosRes] = await Promise.all([
        api.get(`/avaliacoes-loja/${params.id}`),
        api.get(`/comentarios/avaliacao-loja/${params.id}`),
      ]);
  ...
  async function carregarComentarios() {
    try {
      const response = await api.get(`/comentarios/avaliacao-loja/${params.id}`);
  ...
  async function enviarComentario() {
    ...
    try {
      await api.post("/comentarios", {
        ...
        avaliacaoLojaId: Number(params.id),
  ...
  return (
    <main className="bg-[#F6F3E4] min-h-screen">
      ...
      <div className="relative">
        ...
        <div className="absolute top-14 left-14 right-14 flex justify-between">
          <div className="flex items-start gap-6">
            <Link href="/home">
              <Image src="/seta-esquerda.svg" alt="Voltar" width={35} height={55} />
            </Link>
      ...
      {modalAvaliacaoAberto && (
        <ModalEditarAvaliacaoLoja
          avaliacaoId={avaliacao.id}
          lojaId={avaliacao.loja.id}
          nomeLoja={avaliacao.loja.nome}
